...
const srcDir = path.join(__dirname, "../public/img/categories/chauffe-eau");
const outCategory = path.join(__dirname, "../public/img/categories/chauffe-eau-villa.jpg");
const outHero = path.join(__dirname, "../public/img/categories/chauffe-eau-hero.jpg");
...
const CARD_SOURCES = {
  "slim-silver": { cardFill: 0.96 },
  "smart-square": { cardFill: 0.94 },
  "blanc-elcb": { cardFill: 0.96 },
  "mini-eco": { cardFill: 0.95 },
  horizontal: { cardFill: 0.94 },
  "deco-elcb": { cardFill: 0.95 },
  flashheat: { cardFill: 0.92 },
};
...
async function loadSource(key) {
  const files = fs.readdirSync(srcDir).filter((f) => f.startsWith(`${key}-source.`));
  if (!files.length) throw new Error(`missing source for ${key}`);
  const raw = fs.readFileSync(path.join(srcDir, files[0]));
  const png = await cleanWhiteBg(raw);
  fs.writeFileSync(path.join(srcDir, `${key}.png`), png);
  return png;
}

for (const key of GAMMES) {
  try {
...
<svg width="1200" height="820" xmlns="http://www.w3.org/2000/svg">
  <defs>
    <radialGradient id="w" cx="50%" cy="45%" r="58%">
      <stop offset="0%" stop-color="#fde8d8" stop-opacity="0.85"/>
      <stop offset="55%" stop-color="#f5c4a0" stop-opacity="0.35"/>
      <stop offset="100%" stop-color="#fff" stop-opacity="0"/>
    </radialGradient>
  </defs>
  <ellipse cx="600" cy="360" rx="520" ry="290" fill="url(#w)"/>
  <path d="M40,440 C200,300 400,340 600,380 C800,420 1000,340 1160,420" fill="none" stroke="#f0c9a8" stroke-width="30" opacity="0.38" stroke-linecap="round"/>
  <path d="M80,500 C260,390 440,420 620,450 C800,480 980,410 1120,470" fill="none" stroke="#e8a87c" stroke-width="18" opacity="0.25" stroke-linecap="round"/>
  <ellipse cx="600" cy="540" rx="460" ry="22" fill="#e8a87c" opacity="0.1"/>
</svg>`);

const slim = await withShadow(await fitProduct("slim-silver", 340, 520));
const horizontal = await withShadow(await fitProduct("horizontal", 480, 320));

const portrait = await sharp({
  create: { width: 1200, height: 1600, channels: 3, background: { r: 255, g: 255, b: 255 } },
})
  .composite([
    { input: warmSvg, top: 260, left: 0 },
    { input: slim, top: 380, left: 140 },
    { input: horizontal, top: 560, left: 580 },
  ])
  .jpeg({ quality: 95 })
  .toBuffer();
fs.writeFileSync(outCategory, portrait);

const slimL = await withShadow(await fitProduct("slim-silver", 380, 560));
const horizontalL = await withShadow(await fitProduct("horizontal", 520, 360));

const landscape = await sharp({
  create: { width: 2400, height: 1200, channels: 3, background: { r: 255, g: 255, b: 255 } },
})
  .composite([
    { input: warmSvg, top: 80, left: 500 },
    { input: slimL, top: 280, left: 320 },
    { input: horizontalL, top: 360, left: 1180 },
  ])
  .jpeg({ quality: 95 })
  .toBuffer();
fs.writeFileSync(outHero, landscape);

for (const [key, cfg] of Object.entries(CARD_SOURCES)) {
