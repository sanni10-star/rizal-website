...
import type { SolairePvProduct } from "@/content/solaire-marques-photovoltaique";
import { cn } from "@/lib/utils";

type Tab = "produit" | "installation";

export function SolairePvProductCard({
  product,
  marqueTitle,
  accent,
  whatsappHref,
    ...
    <article className="flex flex-col overflow-hidden rounded-3xl border border-ink/8 bg-white shadow-sm">
      <div className="relative">
        <div className="relative aspect-[4/3] w-full overflow-hidden bg-white">
          <ProductImage
            ...
            loading="lazy"
            fit={tab === "produit" ? "contain" : "cover"}
            className="!absolute !inset-0 !block !h-full !w-full !min-h-0 !min-w-0 transition-opacity duration-300"
          />
        </div>
      ...
      <div className="flex flex-1 flex-col gap-3 p-5">
        ...
        <div className="mt-2 flex flex-col gap-2">
          <Link
            href={`/energie-solaire/${product.catalogId}`}
