...
import { SOLAIRE_MARQUES_VARIATEUR_PATH } from "@/content/solaire-marques-variateur";
import { solaireVariateurMarques } from "@/content/solaire-variateur-marques";
import { cn } from "@/lib/utils";

const aspectClass =
  "aspect-[16/9] min-h-[17.5rem] sm:aspect-[21/9] sm:min-h-[min(42vw,22rem)] md:min-h-[min(38vw,26rem)]";

export function SolaireVariateurRubrique() {
  ...
  const active = solaireVariateurMarques[selected];
  ...
  return (
    <div className="mt-10">
      ...
      >
        <div ref={emblaRef} className="h-full overflow-hidden">
          <div className="flex h-full">
            {solaireVariateurMarques.map((marque, index) => {
              ...
              return (
                ...
                >
                  <Link
                    href={`${SOLAIRE_MARQUES_VARIATEUR_PATH}#${marque.id}`}
        ...
        <div className="absolute bottom-4 left-1/2 z-20 flex -translate-x-1/2 gap-1.5">
          {solaireVariateurMarques.map((m, i) => (
            <button
              key={m.id}
              type="button"
      ...
      {active ? (
        <Link
          href={SOLAIRE_MARQUES_VARIATEUR_PATH}
